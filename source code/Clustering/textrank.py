...
word_vectors = insta_data['wv'].to_list() 
num_clusters = 9

# K means 를 정의하고 학습시킨다.
kmeans_clustering = KMeans( n_clusters = num_clusters )
idx = kmeans_clustering.fit_predict( word_vectors )
insta_data['category'] = idx


# In[ ]:


aaa = insta_data.content.loc[insta_data.category == 6]
...
tagged_data = make_doc2vec_data(insta_data, "content", t_document=True)


# In[ ]:


def make_d2v_models(tagged_data,  vector_size=128, window = 3, epochs = 40, min_count = 3, workers = 4):
...
model = make_d2v_models(tagged_data)
...
all_docvecs = model.dv.vectors


# In[ ]:


for i in range(len(all_docvecs)):
    insta_data.dv[i] = all_docvecs[i].tolist()
...
user1 = user.loc[user.category == 7]
user2 = user.loc[user.category == 3]
user3 = user.loc[user.category == 2]


# In[118]:


user1 = user1.sample(10)
...
user_v = np.mean(user_embedding, axis = 0)
...
for i in range(len(insta_data)):
    cos_sim = dot(user_v, wv_matrix[i])/(norm(user_v)*norm(wv_matrix[i]))
    row.append(cos_sim)
matrix.append(row)
