...
def create_model(all_itmes):
    ...
    model = tp.LDAModel(k=8, alpha=0.1, eta=0.01, min_cf=5, rm_top=30, tw=tp.TermWeight.IDF)
    # LDAModel을 생성
...
#     insta_data = pd.read_csv('tot_dataset.csv') #인스타 크롤링 데이터 원본 호출
    ...
    all_itmes = token_data #전처리된 통합 데이터 호출 
    ...
    insta_data.columns = ['item_index', 'content', 'date', 'like', 'tags', 'imgUrl', 'name', 'overlap', 'place'] # 컬럼명 재정의
...
#   insta_data.token_nolist[i] = " ".join(i_data.content[i])
...
embedding_model = Word2Vec(insta_data.content, vector_size=100, window = 2, min_count=3, workers=4, epochs=100, sg=1, seed=0)
...
insta_data['wv'] = insta_data['content'].map(get_sentence_mean_vector)
...
create_model_spherical(insta_data, 10)
...
insta_data.category.value_counts()


# In[138]:


user = insta_data.copy()
...
user1 = user.loc[user.category == 9]
user2 = user.loc[user.category == 8]
user3 = user.loc[user.category == 6]


# In[141]:


user1 = user1.sample(15)
user2 = user2.sample(3)
user3 = user3.sample(2)


# In[142]:


user_info = pd.concat([user1, user2, user3])


# In[148]:


def user_embedding(wv_data):
    user_embedding = []
    for i in wv_data:
        user_embedding.append(i)
    user_embedding = np.array(user_embedding)
    user_vector = np.mean(user_embedding, axis = 0)
    
    return user_vector

def create_user_matrix(user_vector, item_vector, basedata):
    item_vector = np.asarray(item_vector)
    row = []
    user_matrix = []
    for i in range(len(basedata)):
        cos_sim = dot(user_vector, item_vector[i])/(norm(user_vector)*norm(item_vector[i]))
        row.append(cos_sim)
    user_matrix.append(row)
   
    return user_matrix

def user_recommand(user_matrix, basedata):
    matrix = np.array(user_matrix).reshape(-1)
    scores = np.argsort(-matrix)[:10]
...
user_vector = user_embedding(user_info.wv)


# In[150]:


user_matrix = create_user_matrix(user_vector, insta_data.wv, insta_data)


# In[151]:


result = user_recommand(user_matrix, insta_data)
...
temp = result.category.values.tolist()
count = result.category.value_counts()
